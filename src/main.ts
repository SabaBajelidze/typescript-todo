let personName = document.querySelector("#person-name") as HTMLInputElement;
let personCode = document.querySelector("#person-code") as HTMLInputElement;
let loginForm = document.querySelector("#form-login") as HTMLFormElement;
let signUpButton = document.querySelector("#sign-up") as HTMLSpanElement;
let loginDiv = document.querySelector("#login") as HTMLDivElement;
let startOver = document.querySelector("#start-over") as HTMLButtonElement;
...
loginForm.addEventListener("submit", (e) => {
  ...
  let user: Person = persons.find((obj: Person) => {
...
function handleSignUp(
  ...
  e.preventDefault();
  let skinTypes = Array.from(
    document.getElementsByName("skin")
  ) as Array<HTMLInputElement>;
  let eyeColors = Array.from(
    document.getElementsByName("eyes")
  ) as Array<HTMLInputElement>;
  let skinColor = "";
  let eyeColor = "";
...
function completeSignUp(person: Person) {
  let children = Array.from(loginDiv.children);
  children.forEach((el) => {
    el.remove();
  });
...
function createFace(person: Person): HTMLDivElement {
...
function openMenu(person: Person) {
  // Clear Friends Menu
...
function handleEditing(person: Person) {}
